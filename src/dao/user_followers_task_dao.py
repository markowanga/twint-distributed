...
def add_task(task_id: str, username: str, created: datetime, scrap_session_id: str, queue_name: str):
    execute_sql_modify(
        '''INSERT INTO twint_distributed_tasks.UserFollowersScrapTasks(task_id, username, created, finished,
...
def set_task_finished(task_id: str, finished: datetime):
    execute_sql_modify(
        '''UPDATE twint_distributed_tasks.UserFollowersScrapTasks
        SET finished = %s
        WHERE task_id = %s''',
        [finished, task_id])
    return
...
def get_session_id(task_id: str) -> str:
    return execute_sql_query(
        'SELECT * FROM twint_distributed_tasks.UserFollowersScrapTasks WHERE task_id=%s',
        [task_id]
...
def get_all_by_username(username: str):
    return execute_sql_query(
        'SELECT * FROM twint_distributed_tasks.UserFollowersScrapTasks WHERE username=%s',
        [username]
    )
...
def get_all_tasks():
    return execute_sql_query(
        ...
        FROM twint_distributed_tasks.UserFollowersScrapTasks t 
